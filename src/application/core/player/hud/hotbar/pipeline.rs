...
use super::geometry::HotbarVertex;
...
pub(super) fn create(
    ...
    ui_texture_layout: &BindGroupLayout,
) -> RenderPipeline {
    let shader = device.create_shader_module(ShaderModuleDescriptor {
        label: Some("hotbar shader"),
        source: ShaderSource::Wgsl(include_str!("../../../render/shaders/hotbar.wgsl").into()),
    });
    let layout = device.create_pipeline_layout(&PipelineLayoutDescriptor {
        label: Some("hotbar pipeline layout"),
        bind_group_layouts: &[Some(block_texture_layout), Some(ui_texture_layout)],
        immediate_size: 0,
    });
    ...
    device.create_render_pipeline(&RenderPipelineDescriptor {
        ...
        vertex: VertexState {
            ...
            buffers: &[VertexBufferLayout {
                array_stride: std::mem::size_of::<HotbarVertex>() as u64,
